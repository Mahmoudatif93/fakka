...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <h1>@lang('site.confirmpayment')</h1>

            <ol class="breadcrumb">
                <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> @lang('site.dashboard')</a></li>
                <li class="active">@lang('site.confirmpayment')</li>
            </ol>
        </section>
        ...
        <section class="content">
            ...
            <div class="box box-primary">
                ...
                <div class="box-header with-border">
                    ...
                    <h3 class="box-title" style="margin-bottom: 15px">@lang('site.confirmpayment') <small>{{ $clients->total() }}</small></h3>

                    <form action="{{ route('dashboard.confirmpayment.index') }}" method="get">
                    ...
                    @if ($clients->count() > 0)
                        ...
                        <table class="table table-hover">
                            ...
                            <tr>
                                ...
                                <th>@lang('site.pricepergram')</th>
                                <th>@lang('site.total_price')</th>
                                <th>@lang('site.totalfees')</th>
                            ...
                            @foreach ($clients as $index=>$client)
                                <tr>
                                    ...
                                    <td>{{ $client->price_per_gram }}</td>
                                    <td>{{ $client->total_price }}</td>
                                    <td>{{ $client->total_fees }}</td>
                                    <td>@if($client->status==0){{"pending"}} @elseif ($client->status==1){{"Approved"}} @else{{"Rejected"}}@endif</td>

                                    <td>	
                                            <button type="button" class="btn btn-info "
																onclick="get_datiles(<?= $client->id?>)"
                                    ...
                                    <td>
												<button type="button" class="btn btn-primary "
														onclick="$('#username').val('<?= $client->client->first_name . ' ' . $client->client->last_name?>');$('#emailaddress').val('<?= $client->client->email ?>');"
														data-toggle="modal" data-target="#send_email-modal"> Send Email
												</button>
											</td>
                                    <td>
                                     @if (auth()->user()->hasPermission('update_confirmpayment'))
                                          @if($client->status==0)
                                                <form action="{{ route('dashboard.confirmpayment.approve', $client->id) }}" method="post" style="display: inline-block">
                                                    {{ csrf_field() }}
                                                    {{ method_field('post') }}
                                                    <button type="submit" class="btn btn-success"> @lang('site.confirm')</button>
                                                </form>
                                            @elseif ($client->status==1)

                                               <!-- <form action="{{ route('dashboard.confirmpayment.disapprove', $client->id) }}" method="post" style="display: inline-block">
                                                    {{ csrf_field() }}
                                                    {{ method_field('post') }}
                                                    <button type="submit" class="btn btn-danger"> @lang('site.disapproved')</button>
                                                </form>-->
                                                <button class="btn btn-primary btn-sm disabled"><i class="fa fa-trash"></i> @lang('site.approved')</button>
                                            @else
                                            <form action="{{ route('dashboard.confirmpayment.approve', $client->id) }}" method="post" style="display: inline-block">
                                                    {{ csrf_field() }}
                                                    {{ method_field('post') }}
                                        ...
                                        @if (auth()->user()->hasPermission('delete_confirmpayment'))
                                            <form action="{{ route('dashboard.confirmpayment.destroy', $client->id) }}" method="post" style="display: inline-block">
                                                {{ csrf_field() }}
                                                {{ method_field('delete') }}
