...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <h1>@lang('site.virtual_gold')</h1>

        <ol class="breadcrumb">
            <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> @lang('site.dashboard')</a></li>
            <li class="active">@lang('site.virtual_gold')</li>
        </ol>
    </section>
    ...
    <section class="content">
        ...
        <div class="box box-primary">
            ...
            <div class="box-header with-border">
                ...
                <h3 class="box-title" style="margin-bottom: 15px">@lang('site.virtual_gold') <small>{{ $clients->total() }}</small></h3>

                <form action="{{ route('dashboard.virtual_gold.index') }}" method="get">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $client->total_price .' EGP'}}</td>
                            <td>{{ $client->Orderdetails[0]->product_weight .' gm'}}</td>
                            <td>{{ "24" }}</td>
                            <td>@if($client->status==0){{"pending"}} @elseif ($client->status==1){{"Approved"}} @else{{"Rejected"}}@endif</td>


                            <td>
                                <button type="button" class="btn btn-primary " onclick="$('#username').val('<?= $client->client_name ?>');$('#emailaddress').val('<?= $client->email ?>');" data-toggle="modal" data-target="#send_email-modal"> Send Email
                                </button>
                            </td>
                            <td>
                                @if (auth()->user()->hasPermission('update_virtual_gold'))
                                @if($client->status==0 || $client->status==2) 
                                <form action="{{ route('dashboard.virtual_gold.approve', $client->id) }}" method="post" style="display: inline-block">
                                    {{ csrf_field() }}
                                    {{ method_field('post') }}
                                    <button type="submit" class="btn btn-success"> @lang('site.virtual_gold')</button>
                                </form><!-- end of form -->
                                @else
                                ...
                                <form action="{{ route('dashboard.virtual_gold.destroy', $client->id) }}" method="post" style="display: inline-block">
                                    {{ csrf_field() }}
                                    {{ method_field('delete') }}
